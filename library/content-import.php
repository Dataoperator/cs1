...
class ThemeContentImport {
    ...
    public static function contentImportNoticeAction() {
        ...
        <div id="content-import-notice" class="updated">
            <p>
                <?php echo __('Do you want to import Content?', 'clarityshare'); ?>
                &nbsp; &nbsp; &nbsp; &nbsp;
                <a id="import-import-content" class="import-button button" href="<?php echo $import_href; ?>"><?php echo __('Import content', 'clarityshare'); ?></a>
                <a id="import-replace-content" class="<?php if (get_option('np_imported_content') === false) echo 'hidden'; ?> import-button button" href="<?php echo $import_href; ?>"><?php echo __('Replace previously imported content', 'clarityshare'); ?></a>
                <a id="import-hide-notice" class="import-button button" href="#"><?php echo __('Hide notice', 'clarityshare'); ?></a>
            </p>
        </div>
        <style>
            .import-button {
        ...
        <script>
            jQuery(document).ready(function ($) {
                ...
                function bindImportAction(action, btn) {
                    ...
                    var failMsg = <?php echo json_encode(__('An error occurred while importing.', 'clarityshare')); ?>;
                    var captchaKeys;
                    btn.unbind("click").click(function() {
                        ...
                        doAjax(action).done(function (response) {
                            ...
                            if (import_content_can) {
                                successMsg = <?php echo json_encode(__('Content was successfully imported.', 'clarityshare')); ?>;
                                $('#content-import-notice').html('<p>' + successMsg + '</p>' + captchaMsg);
                            } else {
                                successMsg = <?php echo json_encode(__('Please install and activate <a href="themes.php?page=tgmpa-install-plugins&plugin_status=install">Nicepage plugin</a> for import content.', 'clarityshare')); ?>;
    ...
    public static function pluginRequiredNoticeAction() {
        ...
        <div id="content-import-notice" class="notice notice-warning">
            <p><?php _e('Import process required plugin to be active.', 'clarityshare'); ?></p>
            <p><?php $tgmpa->is_plugin_installed('nicepage')
                    ? _e('Please activate plugin to continue.', 'clarityshare')
                    : _e('Please install and activate plugin to continue.', 'clarityshare'); ?></p>
        </div>
        <?php
    }
